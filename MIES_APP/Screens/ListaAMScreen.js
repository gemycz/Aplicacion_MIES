import {Body, Button, Content, Left, ListItem, Right, Text, Thumbnail} from 'native-base';
import React, {Component} from 'react';
import {FlatList, ImageBackground, RefreshControl, ScrollView, StyleSheet} from 'react-native';
                            ...
                            <FlatList
                                ...
                                renderItem={({item}) => (
                                    <ListItem thumbnail onPress={() => this.props.navigation.navigate("Test", {
                                        id: item.am_id,
                                        nombre: item.am_nombres,
                                        apellido: item.am_apellidos,
                                        cedula: item.am_cedula,
                                        domicilio: item.am_domicilio,
                                        autoidentificacion: item.am_etnia,
                                        genero: item.am_genero,
                                        ...
                                        <Body>
                                            ...
                                            <Text note style={{fontSize: 15, color: 'black'}}><Text
                                                style={{fontWeight: 'bold'}}>Edad: </Text>{moment().diff(item.am_fechNac, 'years')}
                                            </Text>
                                            <Text note style={{fontSize: 15, color: 'black'}}><Text
                                                style={{fontWeight: 'bold'}}>Proximo
                                                Test:</Text> {moment(item.am_fechReg).locale('es').add(6, 'months').format('MMMM, YYYY.')}
                                            </Text>
                                        </Body>
                                        <Right>
                                            <Button transparent
                                                    onPress={() => this.props.navigation.navigate("infoAM", {
                                                        id: item.am_id,
                                                        nombre: item.am_nombres,
                                                        apellido: item.am_apellidos,
                                                        cedula: item.am_cedula,
